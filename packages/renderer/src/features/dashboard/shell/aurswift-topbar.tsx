import { ChevronRight, CircleUserRound, Home, Menu, PanelLeft } from "lucide-react";
...
export function AurSwiftTopbar({
  currentViewTitle,
  breadcrumbItems,
  onSidebarToggle,
  isMobile,
  ...
  const toggleLabel = isMobile ? "Open menu" : "Toggle sidebar";
    ...
    <header className="border-b border-primary/30 bg-primary text-primary-foreground shadow-sm">
      <div className="flex min-h-14 items-center justify-between gap-3 px-3 sm:px-4 lg:px-5">
        <div className="flex min-w-0 items-center gap-3">
          ...
          >
            <div className="flex h-7 w-7 items-center justify-center overflow-hidden rounded bg-white/90">
              {!isLogoBroken ? (
                <img
                  src={logoSrc}
                  alt="AurSwift logo"
        ...
        <div className="flex min-w-0 items-center gap-2 rounded-md bg-white/10 px-2.5 py-1.5">
