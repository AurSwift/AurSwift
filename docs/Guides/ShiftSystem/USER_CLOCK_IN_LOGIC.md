...
It is crucial to understand that these are two separate concepts in Aurswift:

1.  **Authentication (Login)**:
    - **Purpose**: Identifies _who_ is using the terminal.
...
2.  **Time Tracking (Clock In)**:
    - **Purpose**: Tracks _attendance_ and payroll hours.
...
_Note: This is configurable via the `roles` table `shiftRequired` field._
...
_The system is designed to streamline the process for scheduled staff._
...
_Used when a staff member works outside their schedule or if auto-clock-in fails._

1.  **Screen**: PIN Entry Screen.
2.  **Action**: User clicks **"CLOCK IN"** button _before_ entering PIN for login.
3.  **System**:
...
**Do Managers/Cashiers need to clock in _before_ login?**
