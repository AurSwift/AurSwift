...
describe('EditRoleDialog', () => {
  it('renders nothing when role is null', () => {
    const { container } = render(
      <EditRoleDialog role={null} open={true} onOpenChange={vi.fn()} onSubmit={vi.fn()} />
    )
  ...
  it('pre-fills form with role data', () => {
    ...
    render(<EditRoleDialog role={role} open={true} onOpenChange={vi.fn()} onSubmit={vi.fn()} />)
  ...
  it('calls onSubmit with roleId and updated data when form submitted', async () => {
    ...
    render(<EditRoleDialog role={role} open={true} onOpenChange={vi.fn()} onSubmit={onSubmit} />)
  ...
  it('calls onOpenChange when Cancel clicked', async () => {
    ...
    const role = createMockRole()
    render(
      <EditRoleDialog role={role} open={true} onOpenChange={onOpenChange} onSubmit={vi.fn()} />
    )
