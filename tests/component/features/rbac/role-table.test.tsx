...
import { createMockRoles } from '../../../shared/factories/rbac.fixture'
...
describe('RoleTable', () => {
  it('renders all role rows when roles provided', () => {
    ...
    render(<RoleTable {...defaultProps} roles={roles} roleUserCounts={roleUserCounts} />)
  ...
  it('shows empty state when no roles', () => {
    ...
    render(<RoleTable {...defaultProps} roles={[]} onAddRole={onAddRole} />)
  ...
  it('shows empty state with search message when searchTerm is set', () => {
    render(<RoleTable {...defaultProps} roles={[]} searchTerm="foo" onAddRole={vi.fn()} />)
