...
vi.mock('@/features/adaptive-keyboard/adaptive-form-field', () => ({
  AdaptiveFormField: (props: any) => {
    const { readOnly: _readOnly, ...rest } = props
...
describe('CreateRoleForm', () => {
  ...
  it('renders all form fields', async () => {
    render(<CreateRoleForm onSubmit={onSubmit} onCancel={onCancel} isLoading={false} />)
    await waitFor(() => {
  ...
  it('submit button is disabled when no permissions selected', async () => {
    render(<CreateRoleForm onSubmit={onSubmit} onCancel={onCancel} isLoading={false} />)
    await waitFor(() => {
  ...
  it('calls onSubmit with correct data on valid submission', async () => {
    ...
    render(<CreateRoleForm onSubmit={onSubmit} onCancel={onCancel} isLoading={false} />)
    await user.type(screen.getByPlaceholderText(/e.g., inventory_specialist/i), 'test_role')
    await user.type(screen.getByPlaceholderText(/e.g., Inventory Specialist/i), 'Test Role')
