...
describe('RoleFilters', () => {
  it('renders search input', () => {
    ...
    render(<RoleFilters searchTerm="" onSearchChange={onSearchChange} />)
    expect(screen.getByPlaceholderText(/search roles by name or description/i)).toBeInTheDocument()
  })
  ...
  it('calls onSearchChange when user types', async () => {
    ...
    const onSearchChange = vi.fn()
    render(<RoleFilters searchTerm="" onSearchChange={onSearchChange} />)
    const input = screen.getByPlaceholderText(/search roles by name or description/i)
    await user.type(input, 'admin')
    expect(onSearchChange).toHaveBeenCalled()
  })

  it('displays current search term as value', () => {
    render(<RoleFilters searchTerm="manager" onSearchChange={vi.fn()} />)
    const input = screen.getByPlaceholderText(/search roles by name or description/i)
