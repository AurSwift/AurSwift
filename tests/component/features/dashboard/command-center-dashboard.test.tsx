...
vi.mock('@/features/dashboard/command-center/hooks/use-dashboard-action-shortcuts', () => ({
  useDashboardActionShortcuts: (...args: unknown[]) => mockUseDashboardActionShortcuts(...args),
}))
...
describe('CommandCenterDashboard', () => {
  ...
  it('renders explicit empty states for D2 and D3 panels', () => {
    ...
    expect(screen.getByText('No activity events are available yet.')).toBeInTheDocument()
    expect(screen.getByText('No product sales data available.')).toBeInTheDocument()
    expect(screen.getByText('No hourly volume data available.')).toBeInTheDocument()
    expect(screen.getByText('No category distribution data available.')).toBeInTheDocument()
  })
})
