...
export function createMockRole(overrides?: Partial<MockRole>): MockRole {
  const id = overrides?.id || `role-${Date.now()}-${Math.random().toString(36).slice(2, 9)}`
  const now = Date.now()
  return {
    id,
 ...
 */
export function createMockUserRole(overrides?: Partial<MockUserRole>): MockUserRole {
  const id = overrides?.id || `ur-${Date.now()}-${Math.random().toString(36).slice(2, 9)}`
  const now = Date.now()
  return {
    id,
