...
export class LoginPage extends BasePage {
  ...
  private readonly numberButton = (digit: string) => `button:has-text("${digit}")`
  private readonly backButton = 'button:has-text("BACK")'
  private readonly deleteButton = 'button:has-text("DELETE")'
  private readonly errorMessage = '.text-red-500, .text-red-600, [class*="text-red"]'
  ...
  async navigate(): Promise<void> {
    ...
    await this.page.waitForFunction(
      () => {
        ...
        return root.children.length > 0 || typeof (window as any).React !== 'undefined'
      },
      { timeout: 30000 }
    )
    ...
    if (!currentHash.includes('/auth')) {
      ...
      await this.page.waitForFunction(() => window.location.hash.includes('/auth'), {
        timeout: 5000,
      })
    }
  ...
  async selectUser(userNameOrIndex: string | number): Promise<void> {
    ...
    if (typeof userNameOrIndex === 'number') {
      // Select by index
      const userButton = this.page.locator(this.userButton).nth(userNameOrIndex)
      ...
      if (matchingIndex === -1) {
        ...
        const userButton = this.page.locator(this.userButton).nth(matchingIndex)
        await userButton.click()
      }
    }
  ...
  async isLoggedIn(timeout: number = 10000): Promise<boolean> {
    try {
      ...
      await this.page.waitForFunction(() => window.location.hash.includes('/dashboard'), {
        timeout,
      })
      return true
    } catch {
   ...
   */
  async quickLogin(userNameOrIndex: string | number, pin: string): Promise<void> {
    await this.navigate()
